...
6. For SQLite-backed apps, confirm that the generated scaffold stores database files under persistent `/app/data` without hiding image migrations under `/app/db`, mounts Active Storage at `/app/storage`, and adds a release script that runs `rails db:prepare`.
...
- if `config/database.yml` shows SQLite in production, confirm that `cpflow generate` emitted persistent `/app/data` and `/app/storage` volumes without mounting over `/app/db`, plus a `rails db:prepare` release script; otherwise keep the default Postgres workload
