  ...
  context "when no configuration exist in the project" do
    it "generates base config files" do
      inside_dir(GENERATOR_PLAYGROUND_PATH) do
        ...
        expect(controlplane_content).not_to include("pre_deletion: bundle exec rails db:drop")
        expect(controlplane_content).not_to include("pre_deletion: DISABLE_DATABASE_ENVIRONMENT_CHECK=1")
