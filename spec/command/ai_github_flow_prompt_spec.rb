...
describe Command::AiGithubFlowPrompt, :enable_validations, :without_config_file do
  ...
  it "prints the AI rollout prompt with the inferred repo-name app prefix" do
    inside_dir(playground.join("sample-project").tap(&:mkpath)) do
      ...
      expect(result[:stdout]).to include("production promotion `.cpflow` checkout ref")
      expect(result[:stdout]).to include("`control_plane_flow_ref` setup input")
      expect(result[:stdout]).to include("temporary explicit ref overrides are for unreleased testing")
      ...
      expect(result[:stdout]).to include("set its autoscaling metric to `disabled`")
      expect(result[:stdout]).to include("enable `capacityAI: true`")
      expect(result[:stdout]).to include("Shared Postgres and other stateful workloads")
      expect(result[:stdout]).to include("supported stateless app/service workloads")
