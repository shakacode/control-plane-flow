...
| [heroku config](https://devcenter.heroku.com/articles/heroku-cli-commands#heroku-config)                       | `cpflow env -a APP_NAME` for app environment variables; `cpflow config -a APP_NAME` for local `.controlplane/controlplane.yml` settings                                              |
...
| [heroku logs](https://devcenter.heroku.com/articles/heroku-cli-commands#heroku-logs)                           | `cpflow logs -a APP_NAME`; add `-w WORKLOAD_NAME` or `-r REPLICA_NAME` to narrow the stream                                                                                           |
...
| [heroku releases](https://devcenter.heroku.com/articles/heroku-cli-commands#heroku-releases)                   | `cpflow latest-image -a APP_NAME` for the latest image tag; `cpflow deploy-image -a APP_NAME` deploys that image, and promotion flows copy/deploy upstream images.                   |
