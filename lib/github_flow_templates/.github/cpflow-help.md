...
| Name | Required | Notes |
| --- | --- | --- |
| `CPLN_ORG_STAGING` | optional for review apps; yes for staging | Overrides the review/staging Control Plane org. Review-app workflows infer this from the review app entry's `cpln_org` in `.controlplane/controlplane.yml` when unset. |
...
| `REVIEW_APP_PREFIX` | optional | Overrides the per-PR review app prefix. Review-app workflows infer this from the single `match_if_app_name_starts_with: true` app in `.controlplane/controlplane.yml` when unset. |
