...
- Shaka rejects fields outside its schema. Keep the GitHub Actions allowlist in `.agents/trusted-actions.yml`; live branch protection and required checks come from GitHub.
...
- Before merging, require the full current-head `gh pr checks` list to be green; require all review threads to be resolved; and require GitHub to report the PR as mergeable and clean.
