...
`.agents/shaka.md` documents the Shaka package versions and a local syntax check. It does not supply trusted policy. The `--local` check validates candidate YAML and fixed command paths; use the trusted `--ref` check below before relying on candidate configuration.
...
- The seam's `version` identifies Shaka's typed schema, not compatibility with predecessor-shaped keys. Under Shaka's pilot migration procedure, keep it at `1` for field changes that reject old keys loudly; use `2` when a valid key changes meaning or default, or when repositories outside the pilot depend on the contract. Upgrade readers before switching, and make predecessor-key readers fail closed.
