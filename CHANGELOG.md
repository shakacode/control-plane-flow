...
- **Fixed `cpflow doctor` to validate only the selected `setup_app_templates`, while retaining the all-template fallback when none are configured, and generated review-app deployments to wait for workload readiness and an accepted HTTP response before reporting a successful GitHub deployment.** [PR 363](https://github.com/shakacode/control-plane-flow/pull/363) by [Justin Gordon](https://github.com/justin808).
