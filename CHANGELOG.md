...
- **Fixed generated review-app deploy and delete comment commands so GitHub verifies the comment author's current repository permission and allows only `write`, `maintain`, or `admin`, failing closed when the permission lookup fails.** Fixes [issue 427](https://github.com/shakacode/control-plane-flow/issues/427).
